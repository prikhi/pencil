    ...
    try {
        ...
        for (var i = 0; i < response.length; i++) {
            ...
            for (var j = 0; j < response[i].files.length; j++) {
                var download_url = response[i].files[j].download_url.replace("http://testvm\.openclipart\.org/cchost/content/","http://www.openclipart.org/people/");
                item.images.push({src: download_url, type: response[i].files[j].file_format_info.mime_type, typeName: this.formatType(response[i].files[j].file_format_info.mime_type), size: response[i].files[j].file_rawsize});
            }
            r.result.push(item);
        }
    } catch (e) {
